...
const addPerson = (name, phone) => {
  ...
  return startConnection()
    ...
    .then((exist) => {
      if (exist) {
        return Promise.reject({scode:409,data:"The person already in database"});
      }
      ...
      return person.save()
      .then(data=>({scode:201,data}))
    })
    .catch((e) => {
        return Promise.reject({scode:500,data:`unable to register ${name} to the phonebook`});
    });
};
const exists = (name) => {
  return startConnection()
    ...
    .catch((e) => {
      console.log("searching data failed");
      throw e;
    });
};
...
const findPerson = (id)=>{
    ...
    .then(person=>{
        if(!person){
            return Promise.reject({scode:404,data:"The person not in phonebook"});
        }
...
const total = ()=>{
    ...
    .catch((e) => {
      return Promise.reject({scode:500,data:"counting people on phonebook failed"});
    });
}

export { getPhonebook, addPerson,findPerson, total,deletePerson };
